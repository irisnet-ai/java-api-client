...
public class ParamSet {
  ...
  * @throws IOException if the JSON Object is invalid with respect to ParamSet
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!ParamSet.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ParamSet.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ParamSet` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      if (jsonObj.get("params") != null && !jsonObj.get("params").isJsonNull()) {
        ...
        if (jsonArrayparams != null) {
          ...
          for (int i = 0; i < jsonArrayparams.size(); i++) {
            Param.validateJsonObject(jsonArrayparams.get(i).getAsJsonObject());
          };
        }
      }
  }
